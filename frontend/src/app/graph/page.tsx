    ...
    <div className="flex flex-col h-screen bg-gray-950 text-gray-100">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-gray-800 flex-shrink-0">
        ...
        <form onSubmit={handleFilter} className="flex items-center gap-2">
          ...
          <button
            type="submit"
            className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm rounded-lg px-3 py-1.5 transition-colors"
          >
            Filter
          </button>
          {service && (
      ...
      <main className="flex-1 min-h-0 relative">
        {loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-gray-950/80 gap-4 text-gray-400">
        ...
        {error && !loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-20 gap-3">
            <div className="bg-red-900/40 border border-red-700 rounded-xl px-6 py-4 text-center max-w-sm">
              ...
              <button
                onClick={() => fetchGraph(service || undefined)}
                className="mt-3 bg-red-700 hover:bg-red-600 text-white text-sm rounded-lg px-4 py-1.5 transition-colors"
              >
                Retry
              </button>
            </div>
        ...
        {!loading && !error && nodes.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-gray-500">
            ...
            <p className="text-lg font-medium text-gray-400">Graph is empty</p>
            <p className="text-sm">
      ...
      {/* ── Footer summary ──────────────────────────────── */}
      <footer className="px-6 py-2 border-t border-gray-800 flex items-center justify-between text-xs text-gray-600 flex-shrink-0">
        <span>
