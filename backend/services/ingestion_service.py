...
class IngestionService:
    ...
    def _check_existing_hash(self, service_name: str, file_path: str) -> str:
        ...
        try:
            ...
            _log.getLogger(__name__).debug("Could not fetch content hash for %s/%s", service_name, file_path)
            return ""
    ...
    def _store_file_hash(self, service_name: str, file_path: str, content_hash: str):
        ...
        try:
            ...
            logger.warning("Could not store content hash for %s/%s: %s", service_name, file_path, e)
    ...
    def _get_git_metadata(self, repo_dir: str, file_path: str) -> dict:
        ...
        try:
            ...
            import logging as _log
            _log.getLogger(__name__).debug("git log failed for %s in %s", file_path, repo_dir)
        return {"hash": "", "date": ""}
    ...
    def _ingest_python_file(self, service_name: str, file_path: str, module_name: str,
        ...
        for ep in endpoints:
            ...
            logger.debug("  [Python] Endpoint  %s %s", ep['method'], ep['path'])
    ...
    def _ingest_ts_js_file(self, service_name: str, file_path: str, module_name: str,
        ...
        try:
            ...
            for ep in endpoints:
                ...
                logger.debug("  [TS/JS]  Endpoint  %s %s", ep['method'], ep['path'])
        except Exception as e:
            logger.warning("Endpoints skipped for %s: %s", fname, e)
        ...
        try:
            for cls in typescript_parser.extract_classes(code, language=lang):
                ...
                logger.debug("  [TS/JS]  Class     %s", cls['name'])
        except Exception as e:
            logger.warning("Classes skipped for %s: %s", fname, e)
        ...
        try:
            ...
            logger.warning("Functions skipped for %s: %s", fname, e)
        ...
        try:
            for sc in typescript_parser.extract_schemas(code, language=lang):
                ...
                logger.debug("  [TS/JS]  Schema    %s", sc['name'])
        except Exception as e:
            logger.warning("Schemas skipped for %s: %s", fname, e)
        ...
        try:
            ...
            logger.warning("Imports skipped for %s: %s", fname, e)
    ...
    def ingest_repository(self, repo_url: str, github_token: str = None):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                logger.info("Fetching source from GitHub API...")
    ...
    def ingest_from_github(
        ...
        """
        logger.info("Starting GitHub API ingestion for %s (branch: %s, incremental: %s)", repo_url, branch, incremental)
        ...
        try:
            ...
            logger.warning("[Ingestion] Webhook setup failed: %s", e)
        ...
        try:
            logger.info("[Ingestion] Starting RAG auto-sync for %s...", service_name)
    ...
    def ingest_multiple_repos(self, repos: list[dict]) -> dict:
        ...
        try:
            logger.info("[Multi-repo] Starting RAG auto-sync...")
            from services.rag_service import rag_service
    ...
    def _discover_cross_repo_deps(self, service_names: list[str]) -> int:
        ...
        try:
            with graph_service.driver.session() as session:
                ...
                if count > 0:
                    logger.info("Discovered %d cross-repo dependencies", count)
                return count
        except Exception as e:
            logger.warning("Cross-repo discovery failed: %s", e)
            return 0
